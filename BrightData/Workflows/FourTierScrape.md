...
- Browser automation capability (Playwright) - optional for Tier 3
- Bright Data MCP available - optional for Tier 4

---

## Workflow Steps

### Step 1: Tier 1 - WebFetch (Fast and Simple)
...
**Prerequisite:** Browser skill must be installed (`pai-browser-skill`)

**Actions:**
```bash
# Use Browser skill CLI
bun run $PAI_DIR/skills/Browser/Tools/Browse.ts [URL]

# Or use Browser skill API
import { PlaywrightBrowser } from '$PAI_DIR/skills/Browser/src/index.ts'
const browser = new PlaywrightBrowser()
await browser.launch({ headless: true })
await browser.navigate('[URL]')
const content = await browser.getVisibleText()
await browser.close()
```
...
**Prerequisite:** Bright Data MCP must be configured in claude_desktop_config.json
...
START
  |
  v
User explicitly requested specific tier?
  |
  +-- Yes --> Skip to that tier
  |
  +-- No --> Start at Tier 1
        |
        v
      ...
      Browser skill available?
        ...
        +-- No --> Skip to Tier 4
        |
        +-- Yes --> Attempt Tier 3 (Browser Automation)
              |
              v
            Success? --> Yes --> Return content
              |
      ...
      Bright Data MCP available?
        ...
        +-- No --> Report failure
        |
        +-- Yes --> Attempt Tier 4 (Bright Data MCP)
              |
              v
            Success? --> Yes --> Return content
              |
...
- If no, skip to Tier 4

**If Tier 3 Fails:**
- Log failure reason
- Check if Bright Data MCP is available
- If yes, proceed to Tier 4
